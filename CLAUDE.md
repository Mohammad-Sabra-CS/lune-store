...
- Tokens in `src/app/globals.css`: night `#101321`, night-soft, gold `#C9A96A`, gold-bright, moon `#F3EDDF`, ivory `#F5F1E8`, wine (women's accent), navy-lune (men's accent). Radius is intentionally sharp (0.25rem) — luxury aesthetic.
- Fonts: Marcellus (display) + Jost (body); Arabic falls through to Amiri + IBM Plex Sans Arabic via the font stack — no per-locale font switching needed.
- Aurora backdrop (`src/components/aurora.tsx` + `.aurora-*` utilities): drifting blurred color curtains + starfield on dark sections; `.aurora-wash` puts a faint tint on ivory sections.
- Scroll reveals via `src/components/reveal.tsx` (IntersectionObserver, no animation lib).
